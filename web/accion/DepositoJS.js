$(function () {
    $('#form').submit(function(e) {
        ...
        if ($('#nombre').val() == "") {
            ...
            datos = {bandera: 1, dep_descri: $("#nombre").val()};
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
                ...
                for (var i = 0; i < json.length; i++) {
                    dataSet.push([json[i].id_deposito, json[i].dep_descri,json[i].id_deposito,]);                   
                }
                console.log(dataSet);
...
$('#form_editar').submit(function(e) {
  ...
  if ($('#nombres').val() == "") {
    ...
    dato = {bandera: 2, id_deposito: $("#codigo").val(),dep_descri: $("#nombres").val()};
    $.ajax({
        url: '/reina/DepositoCRTL',
      method: 'POST',
...
eliminar = function(codigo) {
  swal({
    ...
    if (isConfirm) {
    dat = {bandera: 3, id_deposito: codigo};
      $.ajax({
        url: '/reina/DepositoCRTL',
        method: 'POST',
